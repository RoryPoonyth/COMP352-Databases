...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $updateQuery = "UPDATE `Location` SET
            ...
            `phone_number`=?,`web_address`=?,`max_capacity`=? WHERE location_id = ?";
        // Prepare the statement
        if ($stmt = $conn->prepare($updateQuery)) {
            // Bind the parameters
            // Data types for each field:
            // s = string, d = date (will be passed as string), i = integer, etc.
            $stmt->bind_param("ssssssssii", $location_type, $name, $address, $city,
                $province, $postal_code, $phone_number, $web_address, $max_capacity, $location_id);

            // Execute the query
            if ($stmt->execute()) {
                // If successful, redirect back to family_members.php with a success message
                header("Location: locations.php?status=success&message=SuccessLocationEdit");
            } else {
                // If failed to execute, capture the error
                header("Location: locations.php?status=error&message=ErrorLocationEdit");
            }
        }
